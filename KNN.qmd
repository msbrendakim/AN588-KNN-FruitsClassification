...
Now we're going to apply the KNN algorithm to a real dataset. We've learned the theoretical aspects so far, but now let's see how it actually works in practice!
...
Alright, let’s start by loading our fruit dataset. `The fruit_data_with_colors.txt` file contains measurements like mass, width, height, and color score for fruits. We use `read.delim` with `sep="\t"` because it’s a tab-separated file.

We convert `fruit_label` and fruit_name to factors with `as.factor`. This is key because KNN is a classification algorithm, and factors tell R these are categories (e.g., apple, orange) rather than numbers or text.
...
Then, we split the data into 70% training and 30% testing sets using createDataPartition. The set.seed(123) ensures our split is reproducible, and the split keeps the fruit classes balanced. train_x and train_y are our training features and labels, while test_x and test_y are for testing. This lets us train our model and test it on unseen data to check its generalization.

Output Interpretation: There’s no direct output here, but normalization is critical. Without it, mass would dominate distance calculations, making color score irrelevant. This step ensures all features contribute equally, which is key for accurate fruit classification.
...
Now we’re picking the best K—the number of neighbors KNN considers. We test K values from 1 to 15 (odd numbers: 1, 3, 5, ..., 15) using sapply. For each K, we run knn on the test set, compare predictions to test_y, and calculate accuracy (the fraction of correct predictions). The plot visualizes K versus accuracy, and which.max picks the K with the highest accuracy, stored in best_k.

Output Interpretation: The accuracy plot shows how performance changes with K. The output says best_k = 1, meaning K=1 gives the highest accuracy. But here’s a red flag: K=1 means we’re only looking at the closest neighbor, which can make the model overly sensitive to noise or outliers, potentially overfitting. If the plot shows accuracy dropping as K increases, it suggests our test set might be small or the data is very clean and well-separated. In a real-world fruit sorting system, we’d be cautious about K=1 because new fruits might not match so perfectly. We might prefer a slightly higher K (like 3 or 5) for robustness, even if accuracy dips a bit.
...
Finally, we classify a mystery fruit with mass=160g, width=7.2, height=7.5, and color_score=0.76. We normalize it using the same preproc object to match our training data’s scale, then run knn with best_k=1. The prob=TRUE gives us the confidence (proportion of votes for the predicted class).

The prediction is 3, which maps to “mandarin” (based on label_map). The confidence is 1.0, meaning the closest neighbor (since K=1) is definitely a mandarin. This makes sense if the mystery fruit’s features are very close to a mandarin in the training set, but K=1 means we’re betting everything on one neighbor. In a real-world scenario, like a fruit sorting machine, we’d want a higher K for reliability, as one odd fruit could throw us off. This result shows KNN’s power for quick predictions but also highlights the risk of overfitting with a small K.
