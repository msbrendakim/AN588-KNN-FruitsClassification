---
title: "K-Nearest Neighbors: Fruit Sorting"
format: html
editor: visual
---
...
K-Nearest Neighbors (KNN) is a **non-parametric**, **instance-based learning** algorithm used for classification and regression. In this module, we explore the fundamental logic behind KNN and apply it to a **fruit classification** task using physical characteristics of fruits.
...
KNN operates on a simple principle:\
\> "A data point is classified based on how its neighbors are classified."

For a given test instance, the algorithm:

1.  **Computes the distance** between the instance and all training data points.

2.  **Selects the K-nearest neighbors** based on a chosen distance metric (commonly Euclidean).

3.  **Assigns a class** to the instance based on the **majority vote** among its neighbors (for classification).

This method is effective in **low-dimensional spaces** with well-separated classes.

</div>

## Why Normalize?

KNN uses distance to make predictions. Therefore, features must be normalized to ensure that no single variable (like `mass`) dominates others (like `color_score`).

We used:

```{r}
preProcess(features, method = c("center", "scale"))
```

## Choosing the Best K

We evaluated the accuracy across a range of K values:

```{r}
k_values <- c(1, 3, 5, 7, 9, 11, 13, 15)
```

The accuracy was plotted, and the best `k` was selected based on the highest test accuracy.
